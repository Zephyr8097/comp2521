...
{   
    ...
    if (L->first == NULL) {
        //an empty list: add the new one directly
        L->first = L->last = n;
        L->first->next = NULL;
        
    } else if (L->first->next == NULL) {
        //only one element in the list: compare the new with it
        struct IntListNode *curr = L->first;
        
        if (n->data < L->first->data) {
            //new smaller or equal than it
            L->first = n;
            L->first->next = curr;
        ...
        //multi elements in the list: 3 situations
        struct IntListNode *curr = L->first->next;
        struct IntListNode *before = L->first;
        
        if (n->data <= L->first->data) {
            //new smaller the first
            L->first = n;
            n ->next = before;
            
        } else {
            while (curr != NULL && n->data > before->data) {
            
                if (curr->next == NULL && n->data > curr->data) {
                    //new bigger than the last
                    curr->next = L->last = n;
                    n->next = NULL;
                    curr = curr->next->next; //get out of the loop
                    ...
                    curr = L->last->next; //get out of the loop
                }
            }
        }
    }
    
	L->size++; // make the size of the list fits
}
