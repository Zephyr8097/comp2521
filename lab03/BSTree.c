...
{   
    /*Let the root join the queue and be the current and print it out, then let
      the curr->left join the queue, then let the curr->right join the queue,
      use the QueueIsEmpty function to be the condition of loop, then print out
      the next value in the queue which is Level1 left value and keep looping to
      let all value get into the queue and print them out in the level order*/
    if (t == NULL) {
        //empty tree
        return;
    }
    Queue myQ = newQueue();//build an empty queue.
    QueueJoin(myQ,t);
    while (QueueIsEmpty(myQ) != 1) {
        Item curr = QueueLeave(myQ);
        printf("%d ", curr->value);
        if (curr->left != NULL) {
            QueueJoin(myQ, curr->left);
        }
        if (curr->right != NULL) {
            QueueJoin(myQ, curr->right);
        }
    }
    dropQueue(myQ);
     // TODO
...
int BSTreeNumLeaves (BSTree t)
{
    ...
    if (t->left == NULL && t->right == 0) {
        //only the root in the tree
        return 1;
    } else {
        return BSTreeNumLeaves(t->left) + BSTreeNumLeaves(t->right);
        /*recall the function and keep moving to the leaf node and return 1 and 
          add them together*/
    }
	// TODO
}
