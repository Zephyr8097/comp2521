...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	PlaceType startType = idToType(start);
	int i = 0;
    ...
    while (curr != NULL) {  
        if (idToType(curr->v) == SEA) {
            ...
            while (temp != NULL) {
                if (temp->v == end && ((startType != SEA && idToType(temp->v) != SEA) || (startType != SEA && idToType(temp->v) != LAND))){
                    if (boatAdded == 0) {
                        type[i] = curr->type;
                        i++;
                        boatAdded = 1;
	...
	return numConnections;  // to keep the compiler happy
}
